...
function ShowLoader(a,b){a="object"==typeof a?a:$("#"+a);0<$(a).length&&(1!=b&&1!=b&&"1"!=b||$(a).css("height",Math.max($(document).outerHeight(),window.innerHeight)+"px"),$(a).show())}function HideLoader(a){a="object"==typeof a?a:$("#"+a);0<$(a).length&&$(a).hide()}function OpenUrl(a,b){if(!a)return!1;!0===b||1===b||"1"===b?(window.open(a,"_blank"),window.focus()):window.open(a)}
...
function InitTCBOFancyTree(a,b,c,d,e,f,g,m,k,p){if(a&&0!==a.length){var l=encodeURIComponent(b),h=xAppWebLink+"/aindex.php?namespace="+f;b="";k=k?"&hpc=1":"";if(g||0<g.length)b+="&uid="+g;g="";if("object"===typeof e)for(var n in e)g+="&"+n+"="+e[n];else"string"===typeof e&&(g=e);h=1===m||!0===m?h+("&arhash="+encodeURIComponent(GibberishAES.enc("module="+c+"&method="+d+g+k+b+"&phash="+window.name,"xJS"))):h+("&module="+c+"&method="+d+g+k+b+"&phash="+window.name);$("#"+a+"-ctree").fancytree({checkbox:!0,
icon:p||!1,selectMode:1,clickFolderMode:1,debugLevel:0,source:{url:h+"&type=json&tree=1&val="+l},lazyLoad:function(a,b){b.result={url:h+"&type=json&tree=1&val="+l,data:{key:b.node.key}}},createNode:function(a,b){if(!b.node.data.hasSelectedChild)return!1;$.ajax({url:h+"&type=json&tree=1&val="+l,data:{key:b.node.key},dataType:"json",success:function(a){b.node.addChildren(a)}})},select:function(b,c){c.node.isSelected()?(TCBOSetValue(a,c.node.key,c.node.title,!1),CBODDBtnClick(a)):TCBOSetValue(a,"","",
!1)}})}}$(document).on("keydown","input.clsGridComboBox[type=text]",function(a){if($("#"+$(this).attr("data-id")).attr("disabled"))return a.preventDefault(),!1;13==a.keyCode?(a=$(this).attr("data-id"),$("#"+a+"-dropdown").attr("data-reload",1),GCBODDBtnClick(a,1)):27==a.keyCode?(a.preventDefault(),GCBODDBtnClick($(this).attr("data-id"),0)):(a.altKey&&(40==a.keyCode||98==a.keyCode)||115==a.keyCode)&&GCBODDBtnClick($(this).attr("data-id"),null)});
