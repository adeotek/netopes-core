...
d){for(var e=$jscomp.makeIterator(a),f=e.next();!f.done;f=e.next())c(f.value).callWhenSettled_(b,d)})};e.all=function(a){var b=$jscomp.makeIterator(a),d=b.next();return d.done?c([]):new e(function(a,e){function f(b){return function(c){g[b]=c;m--;0==m&&a(g)}}var g=[],m=0;do g.push(void 0),m++,c(d.value).callWhenSettled_(f(g.length-1),e),d=b.next();while(!d.done)})};return e},"es6","es3");
...
function InitFancyTree(a,b,c,d,e,f,g,p,h,l){if(a&&0!==a.length){var k=nAppBaseUrl+"/aindex.php?namespace="+e;e="";h=h?"&hpc=1":"";if(f||0<f.length)e+="&uid="+f;f="";if("object"===typeof d)for(var n in d)f+="&"+n+"="+d[n];else"string"===typeof d&&(f=d);k=1===g||!0===g?k+("&arhash="+encodeURIComponent(GibberishAES.enc("module="+b+"&method="+c+f+h+e+"&phash="+window.name,"xJS"))):k+("&module="+b+"&method="+c+f+h+e+"&phash="+window.name);$("#"+a).fancytree({checkbox:p||!1,icon:l||!1,selectMode:1,clickFolderMode:1,
debugLevel:0,source:{url:k+"&response_type=json&tree=1"},lazyLoad:function(a,b){b.result={url:k+"&response_type=json&tree=1",data:{key:b.node.key}}},createNode:function(a,b){if(!b.node.data.hasSelectedChild)return!1;$.ajax({url:k+"&response_type=json&tree=1",data:{key:b.node.key},dataType:"json",success:function(a){b.node.addChildren(a)}})},click:function(a,b){$(this).trigger("fancyTree.onclick",b)},dblclick:function(a,b){$(this).trigger("fancyTree.ondblclick",b)},select:function(a,b){$(this).trigger("fancyTree.onselect",
...
function SetFieldErrorClass(a,b,c){if(!a)return!1;c=c?c:"clsFieldError";a=$(a);b?b=!0:(b=a.hasClass("clsNumericTextBox")?0!==GetNumericTextBoxValue(a):a.hasClass("clsSmartComboBox")?!!GetSmartCBOValue(a.attr("id")):a.val()&&0<a.val().length,a.hasClass("clsCkEditor")&&(a=a.parent().find("#cke_"+a.attr("id")+" .cke_contents .cke_wysiwyg_div")));b?(a.removeClass(c),a.hasClass("clsSmartComboBox")&&(b=a.data("select2"))&&b.$selection.removeClass(c)):(a.addClass(c),a.hasClass("clsSmartComboBox")&&(b=a.data("select2"))&&
b.$selection.addClass(c));return!0}function AddClassOnErrorByParent(a,b,c){$("#"+a+" .clsRequiredField").each(function(){$(this).hasClass("select2-selection")||SetFieldErrorClass(this,b,c)})}function AddClassOnError(a,b,c){SetFieldErrorClass($("#"+a),b,c)}function AddClassOnErrorByName(a,b,c,d){$("#"+a+' [name="'+b+'"]').each(function(){$(this).hasClass("select2-selection")||SetFieldErrorClass(this,c,d)})}function CheckIfEnter(a){a&&a.which?a=a.which:(a=event,a=a.keyCode);return 13==a?!0:!1}
...
function GetCkEditorData(a){"object"===typeof a&&(a=a.getAttribute("id"));return(a=CKEDITOR.instances[a])?a.getData():""}function SetCkEditorData(a,b){"object"===typeof a&&(a=a.getAttribute("id"));(a=CKEDITOR.instances[a])&&a.setData(b)}function SetCkEditorReadonly(a,b){"object"===typeof a&&(a=a.getAttribute("id"));(a=CKEDITOR.instances[a])&&a.setReadOnly(!0===b||"true"===b||1===b||"1"===b)}
function AppendCkEditorData(a,b){"object"===typeof a&&(a=a.getAttribute("id"));(a=CKEDITOR.instances[a])&&a.insertHtml(b)}
...
start:function(a){(a=$(c).attr("data-statusid"))&&$("#"+a).length&&$("#"+a).css("display","")},done:function(a,b){(a=$(c).attr("data-statusid"))&&$("#"+a).length&&$("#"+a).css("display","none");if(b.result.files[0].error&&""!==b.result.files[0].error)ShowErrorDialog("Upload failed: "+b.result.files[0].error);else if(a=$(c).attr("data-callback"))a=GibberishAES.dec(decodeURIComponent(a),"HTML"),a instanceof Function?a(b.result.files[0].name,b.files[0].name,this):(a=a.replace("&amp;namespace=","&namespace="),
a=a.replaceAll("#uploadedFile#",b.result.files[0].name),a=a.replaceAll("#originalFile#",b.files[0].name),eval(a))},fail:function(a,b){ShowErrorDialog("Upload failed: "+b.errorThrown+" (Status: "+b.textStatus+")")}})}else return!1}
