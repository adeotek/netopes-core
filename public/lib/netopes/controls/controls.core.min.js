...
(function(a){a.fn.imageCheckBoxCheck=function(){this.filter('input[type="text"]').each(function(){a(this).val(1);a(this).trigger("change")});return this};a.fn.imageCheckBoxUncheck=function(){this.filter('input[type="text"]').each(function(){a(this).val(0);a(this).trigger("change")});return this};a.fn.imageCheckBoxToggle=function(){this.filter('input[type="text"]').each(function(){"1"===a(this).val()?a(this).val(0):a(this).val(1);a(this).trigger("change")});return this}})(jQuery);
function GroupCheckBoxBaseEvent(a){var b=$(a).data("multiple")?1==$(a).data("multiple"):!1;if(a&&(1!=$(a).val()||0!=b)){var c=$(a).attr("data-id");c&&0<$("#"+c).length&&(b?(b=[],b=""==$("#"+c).val().trim()?[]:$("#"+c).val().trim().split("|"),1==$(a).val()?($(a).val(0),b=b.filter(function(b){return b!=$(a).data("val")})):($(a).val(1),b.push($(a).data("val"))),$("#"+c).val(b.join("|"))):($("#"+c).val($(a).data("val")),$("#"+c+"-container input[type=image].clsGCKBItem").val("0"),$(a).val("1")),$("#"+
...
(a=$("#"+a).attr("data-onchange"))&&0<a.length&&eval(a)}function TreeCBODDBtnClick(a){if(!$("#"+a+"-cbo").prop("disabled")){var b=$("#"+a+"-dropdown");if("none"==$(b).css("display")){var c=$(b).width(),d=$("#"+a+"-cbo").outerWidth(),e=$("#"+a+"-cbo").position();a=e.top+$("#"+a+"-cbo").outerHeight();e=e.left;e+c>window.innerWidth&&window.innerWidth>c&&(e=Math.max(0,e-(c-d)));$(b).css("top",a+"px");$(b).css("left",e+"px");$(b).show()}else $(b).hide()}}
function TreeCBOSetDisabled(a,b,c){if("string"!==typeof a||0>=a.length)return!1;var d=$("#"+a+"-cbo");if(!d||0>=d.length)return!1;!0===b||1===b||"1"===b?(d.prop("disabled","disabled"),d.removeClass("stdro"),$("#"+a+"-dropdown").hide()):(d.prop("disabled",!1),d.addClass("stdro"));!0!==c&&1!==c&&"1"!==c||TreeCBOClear(a);return!0}function TreeCBOClear(a,b){b&&$("#"+a+"-cbo").prop("disabled")||TreeCBOSetValue(a,"","",!0)}
function TreeCBOSetValue(a,b,c,d,e){var f=$("#"+a).val();$("#"+a).val(b);var g=$("#"+a+"-cbo");$(g).val(c);$(g).attr("data-value",b);if($("#"+a).attr("disabled"))return!1;if(!0===e||1==e)$("#"+a+"-ctree").fancytree("getTree").reload();else if(!0===d||1==d)b=$("#"+a+"-ctree").fancytree("getTree").getNodeByKey(f),null!=b&&b.setSelected(!1);(a=$("#"+a).attr("data-onchange"))&&0<a.length&&eval(a)}
...
"xJS"))):p+(m+a+"&phash="+window.name);return p+"&response_type=json&tree=1&val="+n};$("#"+a+"-ctree").fancytree({checkbox:!0,icon:h||!1,selectMode:1,clickFolderMode:1,debugLevel:0,source:function(){return{url:r(),cache:!1}},lazyLoad:function(a,b){a=r();b.result={url:a,data:{key:b.node.key}}},createNode:function(a,b){if(!b.node.data.hasSelectedChild)return!1;a=r();$.ajax({url:a,data:{key:b.node.key},dataType:"json",success:function(a){b.node.addChildren(a)}})},select:function(b,c){c.node.isSelected()?
(TreeCBOSetValue(a,c.node.key,c.node.title,!1),CBODDBtnClick(a)):TreeCBOSetValue(a,"","",!1)}})}}
...
function SmartCBOInitialize(){$("select.SmartCBO").each(function(a,b){if(!$(b).hasClass("select2-hidden-accessible")){var c=$(b).attr("id"),d=$(b).attr("data-smartcbo");if(c&&d){a={};c=GibberishAES.dec(decodeURIComponent(d),c);try{a=eval(c)}catch(e){console.log(e),console.log(c)}$(b).select2(a)}}})}function GetSmartCBOValue(a,b){if(!a)return null;var c=a;"object"!=typeof a&&(c=$("#"+a));if(!$(c).length)return null;a=$(c).val()||"";return Array.isArray(a)&&!0!==b&&1!=b?a=a.join(","):a}
...
function SetFieldErrorClass(a,b,c){if(!a)return!1;c=c?c:"clsFieldError";a=$(a);b||(a.hasClass("clsNumericTextBox")?0!==GetNumericTextBoxValue(a):a.hasClass("clsSmartComboBox")?GetSmartCBOValue(a.attr("id")):a.val()&&0<a.val().length)?(a.removeClass(c),a.hasClass("clsSmartComboBox")&&a.data("select2").$selection.removeClass(c)):(a.addClass(c),a.hasClass("clsSmartComboBox")&&a.data("select2").$selection.addClass(c));return!0}
function AddClassOnErrorByParent(a,b,c){$("#"+a+" .clsRequiredField").each(function(){$(this).hasClass("select2-selection")||SetFieldErrorClass(this,b,c)})}function AddClassOnError(a,b,c){SetFieldErrorClass($("#"+a),b,c)}function AddClassOnErrorByName(a,b,c,d){$("#"+a+' [name="'+b+'"]').each(function(){$(this).hasClass("select2-selection")||SetFieldErrorClass(this,c,d)})}function CheckIfEnter(a){a&&a.which?a=a.which:(a=event,a=a.keyCode);return 13==a?!0:!1}
$(document).on("keydown",".clsSetNoNumericValidation",function(a){var b=a.which||a.keyCode||a.charCode||0;(48<=b&&57>=b||96<=b&&105>=b)&&a.preventDefault()});
...
function GetCkEditorData(a){"object"===typeof a&&(a=a.getAttribute("id"));return(a=CKEDITOR.instances[a])?a.getData():""}function SetCkEditorData(a,b){"object"===typeof a&&(a=a.getAttribute("id"));(a=CKEDITOR.instances[a])&&a.setData(b)}
