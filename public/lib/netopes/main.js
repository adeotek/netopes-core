...
function arrayMerge(initialData,newData,recursive) {
    if(typeof (initialData)!=='object' && !Array.isArray(initialData)) { return newData; }
    if((typeof (newData)!=='object' && !Array.isArray(newData))
        || (typeof (initialData)==='object' && typeof (newData)!=='object')
        || (Array.isArray(initialData) && !Array.isArray(newData))) { return initialData; }
    let isRecursive=!!recursive;
    if(Array.isArray(initialData)) {
        newData.forEach((item,key) => { initialData.push(item); });
    } else {
        for(let key in newData) {
            if(!newData.hasOwnProperty(key)) { continue; }
            if(initialData.hasOwnProperty(key) && (typeof (newData[key])==='object' || Array.isArray(newData[key])) && isRecursive) {
                initialData[key]=arrayMerge(initialData[key],newData[key],true);
            } else {
