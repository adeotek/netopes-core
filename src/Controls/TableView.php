...
class TableView {
    ...
    protected function GetActionCommand($type = '',$params = NULL,$process_call = TRUE) {
        ...
        if(!$exec_callback || !strlen($js_callback)) {
            return NApp::Ajax()->LegacyPrepare($call,$this->loader);
        }
        return NApp::Ajax()->LegacyPrepareWithCallback($call,$js_callback,$this->loader);
            ...
            case 'actions':
                ...
                foreach($v['actions'] as $act) {
                    ...
                    if($acommand){
                        ...
                        $aparams['onclick']=NApp::Ajax()->LegacyPrepare($acommand,$this->loader);
                    }//if($acommand)
                ...
                if(is_null($cValue) && isset($defaultDisplayValue)) {
                    ...
                    if(is_array($p_pafreq) && count($p_pafreq)) {
                        foreach($p_pafreq as $pr) {
                            ...
                            $c_params[$pr]=NApp::Ajax()->LegacyPrepare($c_params[$pr],$this->loader);
                        }//END foreach
                    }//if(is_array($p_pafreq) && count($p_pafreq))
