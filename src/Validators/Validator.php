 ...
 *
 * @package NETopes\Core\Validators
 * @method static ConvertTimestampToDatetime($value)
 */
class Validator {
    /**
    ...
    public static function GetConverterAdapter(?string $method=NULL): string {
        ...
        if(!strlen($method)) {
            if(!class_exists($class)) {
                throw new AppException('Invalid converter adapter class ['.$class.']!');
            }
            return $class;
        }//if(!strlen($method))
        if(!class_exists($class) || !method_exists($class,$method)) {
            throw new AppException('Invalid converter adapter class/method ['.$class.'::'.$method.']!');
        }
        return $class.'::'.$method;
    ...
    public static function SetConverterAdapter(?string $converterAdapter): void {
        if(isset($converterAdapter) && !class_exists($converterAdapter) && is_subclass_of($converterAdapter,ConverterAdapter::class)) {
            throw new AppException('Invalid converter adapter class!');
        }
    ...
    public static function GetValidatorAdapter(?string $method=NULL): string {
        ...
        if(!strlen($method)) {
            if(!class_exists($class)) {
                throw new AppException('Invalid validator adapter class ['.$class.']!');
            }
            return $class;
        }//if(!strlen($method))
        if(!class_exists($class) || !method_exists($class,$method)) {
            throw new AppException('Invalid validator adapter class/method ['.$class.'::'.$method.']!');
        }
        return $class.'::'.$method;
    ...
    public static function SetValidatorAdapter(?string $validatorAdapter): void {
        if(isset($validatorAdapter) && !class_exists($validatorAdapter) && is_subclass_of($validatorAdapter,ValidatorAdapter::class)) {
            throw new AppException('Invalid validator adapter class!');
        }
    ...
    public static function GetFormatterAdapter(?string $method=NULL): string {
        ...
        if(!strlen($method)) {
            if(!class_exists($class)) {
                throw new AppException('Invalid formatter adapter class ['.$class.']!');
            }
            return $class;
        }//if(!strlen($method))
        if(!class_exists($class) || !method_exists($class,$method)) {
            throw new AppException('Invalid formatter adapter class/method ['.$class.'::'.$method.']!');
        }
        return $class.'::'.$method;
    ...
    public static function SetFormatterAdapter(?string $formatterAdapter): void {
        if(isset($formatterAdapter) && !class_exists($formatterAdapter) && is_subclass_of($formatterAdapter,FormatterAdapter::class)) {
            throw new AppException('Invalid formatter adapter class!');
        }
    ...
    public static function __callStatic($name,$arguments) {
        if(substr($name,0,6)==='Format') {
            ...
            $adapter=static::GetConverterAdapter(substr($name,7));
        } else {
    ...
    public static function ValidateValue($value,$defaultValue=NULL,?string $validation=NULL,?string $sourceFormat=NULL,bool &$isValid=FALSE) {
        ...
        if(!method_exists($adapter,'Validate')) {
            throw new AppException('Invalid validator adapter method ['.$adapter.'::Validate]!');
        }
