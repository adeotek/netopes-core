    ...
    public function MySqlPrepareQuery(&$query,$params=[],$out_params=[],$type='',$firstrow=NULL,$lastrow=NULL,$sort=NULL,$filters=NULL,&$raw_query=NULL,&$bind_params=NULL,$transaction=NULL) {
        ...
        if(is_array($filters)) {
            ...
            foreach($filters as $k=>$v) {
                if(!is_numeric($k) && $k=='table__alias') {
                    continue;
                }
                if(is_array($v)) {
                    if(count($v)==4) {
        ...
        if(is_array($sort)) {
            foreach($sort as $k=>$v) {
                $sort_str.=($sort_str ? ' ,' : ' ').str_replace('"','`',strtolower($k)).' '.$v;
            }
            ...
            if(is_object($result)) {
                if(!$firstrow) {
                    ...
                    if(!$lastrow) {
                        while($data=$result->fetch_array(MYSQLI_ASSOC)) {
                            $final_result[]=$data;
                            if(count($final_result)>=$firstrow) {
                                break;
                            }
                        }//END while
                    } else {
    ...
    public function MySqlEscapeString($param) {
        ...
        if(is_array($param)) {
            ...
            foreach($param as $k=>$v) {
                $result[$k]=$this->connection->escape_string($v);
            }
        } else {
            $result=$this->connection->escape_string($param);
        }
        return $result;
