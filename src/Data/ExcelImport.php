...
class ExcelImport {
    ...
    public function __construct(array $fields,array $params=[]) {
        if(!count($fields)) {
            throw new AppException('Invalid ExcelImport fields list!',E_ERROR,1);
        }
        $this->fields=$fields;
    ...
    public function ProcessFile(string $file,array $params=[],?string $file_type=NULL): void {
        if(!$file || !file_exists($file)) {
            throw new AppException('Invalid input file!',E_ERROR,1);
        }
        if(strlen($file_type)) {
        ...
        if(!strlen($file_type)) {
            throw new AppException('Invalid input file type!',E_ERROR,1);
        }
        if(!in_array($file_type,array_keys($this->file_types))) {
            throw new AppException('Unsupported file type!',E_ERROR,1);
        }
        ...
        foreach($this->fields as $k=>$v) {
            if(isset($v['column']) && strlen($v['column'])) {
                continue;
            }
        ...
        if($max_rows>0) {
            ...
            foreach($this->sheet->getRowIterator($start_row,$end_row) as $row) {
                $this->ReadLine($row,$rowno);
            }
        } else {
            foreach($this->sheet->getRowIterator($start_row) as $row) {
                $this->ReadLine($row,$rowno);
            }
        }//if($max_rows>0)
    ...
    protected function ReadLine(Row &$row,int &$rowno): void {
        $rdata=['_has_error'=>0,'_error'=>'','_rowno'=>$row->getRowIndex()];
        $rowno++;
        try {
            $k=0;
            $eno=0;
    ...
    protected function ProcessDataRow(array &$row): bool {
        if(!$this->send_to_db || !count($row) || !isset($row['_has_error']) || $row['_has_error']==1) {
            return FALSE;
        }
        try {
