...
class ExcelExport {
     ...
     */
    protected $file_types=['xlsx'=>'Xlsx','xls'=>'Xls','ods'=>'Ods','csv'=>'Csv'/*,'xml'=>'Xml','html'=>'Html','htm'=>'Html'*/];
    /**
    ...
    protected $default_formats=[
        'header'=>['align_h'=>'h_center','bold'=>TRUE,'background_color'=>'D8D8D8'],
        'footer'=>['bold'=>TRUE,'background_color'=>'DBEEF3'],
        'standard'=>['align_h'=>'h_general'],
        'left'=>['align_h'=>'h_left'],
        'center'=>['align_h'=>'h_center'],
        'right'=>['align_h'=>'h_right'],
        'number0_c'=>['align_h'=>'h_center','number'=>'#,##0'],
        'number0_r'=>['align_h'=>'h_right','number'=>'#,##0'],
        'number2_c'=>['align_h'=>'h_center','number'=>'#,##0.00'],
        'number2_r'=>['align_h'=>'h_right','number'=>'#,##0.00'],
        'number3_c'=>['align_h'=>'h_center','number'=>'#,##0.000'],
        'number3_r'=>['align_h'=>'h_right','number'=>'#,##0.000'],
        'decimal2_c'=>['align_h'=>'h_center','number'=>'#,##0.00'],
        'decimal2_r'=>['align_h'=>'h_right','number'=>'#,##0.00'],
        'decimal3_c'=>['align_h'=>'h_center','number'=>'#,##0.000'],
        'decimal3_r'=>['align_h'=>'h_right','number'=>'#,##0.000'],
        'euro0_r'=>['align_h'=>'h_right','number'=>'#,##0 €'],
        'euro2_r'=>['align_h'=>'h_right','number'=>'#,##0.00 €'],
        'percent0_c'=>['align_h'=>'h_center','number'=>'#,##0 %'],
        'percent0_r'=>['align_h'=>'h_right','number'=>'#,##0 %'],
        'percent2_c'=>['align_h'=>'h_center','number'=>'#,##0.00 %'],
        'percent2_r'=>['align_h'=>'h_right','number'=>'#,##0.00 %'],
        'percent0x100_c'=>['align_h'=>'h_center','number'=>'#,##0 %'],
        'percent0x100_r'=>['align_h'=>'h_right','number'=>'#,##0 %'],
        'percent2x100_c'=>['align_h'=>'h_center','number'=>'#,##0.00 %'],
        'percent2x100_r'=>['align_h'=>'h_right','number'=>'#,##0.00 %'],
        'date_c'=>['align_h'=>'h_center','datetime'=>'dd.mm.yyyy'],
        'datetime_c'=>['align_h'=>'h_center','datetime'=>'dd.mm.yyyy hh:mm:ss'],
    ];
    /**
    ...
    public function __construct(array $params) {
        if(!count($params) || !array_key_exists('layouts',$params) || !is_array($params['layouts']) || !count($params['layouts'])) {
            throw new AppException('ExcelExport: Invalid parameters !',E_ERROR,1);
        }
        ...
        if(!in_array($file_type,array_keys($this->file_types))) {
            throw new AppException('ExcelExport: Invalid output file type!',E_ERROR,1);
        }
        ...
        foreach($params['layouts'] as $layout) {
            ...
            if($sheet_index<0 || $c_sheet_name!=$sheet_name) {
                ...
                $row_no=1;
                $this->total_row=[];
                if($sheet_index>0 || $this->obj->getSheetCount()==0) {
            ...
            foreach($layout['columns'] as $k=>$column) {
                ...
                if($c_width && strpos($c_width,'%')===FALSE) {
                    if(strpos($c_width,'px')!==FALSE) {
                        $c_width=str_replace('px','',trim($c_width));
                    }
            ...
            if(!is_array($layout['data']) || !count($layout['data'])) {
                continue;
            }
            ...
            if(count($this->total_row)>0) {
                ...
                foreach($this->total_row as $c=>$v) {
                    if(!get_array_value($layout['columns'][$v['key']],'summarize',FALSE,'bool')) {
                        continue;
                    }
            ...
            if(!array_key_exists('freeze_pane',$layout) || $layout['freeze_pane']) {
