...
class MongoDbDbAdapter extends DataAdapter {
	...
	public function MongoDbExecuteQuery($query,$params = [],$out_params = [],$tran_name = NULL,$type = '',$firstrow = NULL,$lastrow = NULL,$sort = NULL,$filters = NULL,$log = TRUE) {
		...
		if(strlen($tran_name)>0) {
			...
			if(!array_key_exists($transaction,$this->transactions) || !isset($this->transactions[$transaction]) || !is_resource($this->transactions[$transaction])) { $this->MongoDbBeginTran($transaction); }
		}//if(strlen($tran_name)>0)
		$final_result = NULL;
		try {
			$result = $this->connection->command($query,array('timeout'=>$this->timeout));
		} catch(MongoException $e) {
		...
		try {
			if(is_object($result) && is_a($result,'MongoCursor')) {
				while($result->hasNext()) { $final_result[] = $result->getNext(); }
			}else{
				$final_result = $result;
			}//if(is_object($result) && is_a($result,'MongoCursor'))
		} catch(Exception $e) {
