...
class ApiDataProvider {
    ...
    public static function Call($params=NULL) {
        ...
        if(array_key_exists('target',$params) && is_array($params['target']) && count($params['target'])>0 && $params['target']['id']) {
            ...
            $targets=DataProvider::GetArray('Api\Native','GetApiTargets',['for_type'=>$ttype,'for_id'=>$tid]);
        ...
        foreach($targets as $target) {
            ...
            if(!$error && $response===FALSE) {
                $error='Unknown API comunication error!';
            }
            if(!$error) {
                /*if(NApp::$debug) {
            ...
            $result[]=$target;
            if(NApp::$debug && $error) {
                echo '<strong>Error:</strong><br/>'.$error.'<br/></br/>';
            }
        }//END foreach
        if($singletarget) {
