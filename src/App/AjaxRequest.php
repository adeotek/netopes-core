...
class AjaxRequest extends \PAF\AjaxRequest {
	...
	public function AjaxRequest($window_name,$module,$method,$params = NULL,$target = NULL,$non_custom = 0,$reset_session_params = 0) {
		if(!strlen($window_name)) { $this->ExecuteJs("window.name = '{$this->app_object->phash}'"); }
		try {
			$olduserid = $this->app_object->GetPageParam('user_id');
			$userid = $this->app_object->GetParam('user_id');
			if($olduserid && $userid!=$olduserid) {
				$this->app_object->SetPageParam('user_id',$userid);
				$this->ExecuteJs("window.location.href = '".$this->app_object->GetAppWebLink()."';");
			}//if($olduserid && $userid!=$olduserid)
			$this->app_object->SetPageParam('user_id',$userid);
			...
			if(strlen(NApp::$aapp_areq_js_callbak)) { $this->ExecuteJs(NApp::$aapp_areq_js_callbak); }
	...
	public function ControlAjaxRequest($window_name,$controlhash,$method,$params = NULL,$control = NULL,$viapost = 0) {
		...
		try {
			...
			if($viapost) {
				...
				$lcontrol = $this->app_object->GetPageParam($controlhash);
	...
	public function SessionLogout($window_name,$namespace) {
		$app_web_link = $this->app_object->GetAppWebLink();
	...
	public function AjaxSetLanguage($window_name,$selected_lang) {
		...
		$old_lang = $this->app_object->GetLanguageCode();
		$this->app_object->SetPageParam('lang_code',$alang[1]);
		$this->app_object->SetPageParam('id_lang',$alang[0]);
