...
class Debugger {
    ...
    public function __construct($debug,$logsPath=NULL,$tmpPath=NULL,$consolePassword=NULL) {
        ...
        if(strlen($consolePassword)) {
            $this->jsConsolePassword=$consolePassword;
        }
        if(strlen($logsPath)) {
    ...
    protected function LoggerInit($browser_type,$tmpPath=NULL) {
        if(!is_string($browser_type) || !strlen($browser_type) || !is_array($this->debugExtensions) || !count($this->debugExtensions) || !array_key_exists($browser_type,$this->debugExtensions) || !is_array($this->debugExtensions[$browser_type]) || !count($this->debugExtensions[$browser_type])) {
            return;
        }
            ...
            switch($dk) {
                case 'PhpConsole':
                    ...
                    if(Connector::getInstance()->isActiveClient()) {
                        ...
                        if(isset($this->jsConsolePassword) && strlen($this->jsConsolePassword)) {
                            $this->debugObjects[$dk]->setPassword($this->jsConsolePassword);
                        }
                    } else {
