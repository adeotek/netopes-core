...
class ErrorHandler implements NETopes\Core\App\IErrorHandler {
	...
	public static function ShowErrors(AppException $exception = NULL) {
		...
		if(count(self::$errorsStack)==1) {
			...
			if(class_exists('NApp')) { NApp::Log2File(array('type'=>'error','message'=>$errMessage.' >>> ','no'=>$errNo,'file'=>$errFile,'line'=>$errLine),self::$errorLogPath.self::$errorLogFile); }
		...
		self::$errorsStack = NULL;
		if(class_exists('NApp')) { NApp::Log2File(array('type'=>'error','message'=>$errMessage),self::$errorLogPath.self::$errorLogFile); }
