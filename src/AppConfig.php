...
class AppConfig {
    ...
    public static function LoadConfig(array $data,array $customStructure) {
        ...
        if(!isset($_NAPP_CONFIG_STRUCTURE) || !is_array($_NAPP_CONFIG_STRUCTURE)) {
            die('Invalid NETopes configuration structure!');
        }
    ...
    public static function GetValue(string $name) {
        if(!is_array(self::$structure)) {
            throw new AppException('Invalid configuration structure!');
        }
        $element=get_array_value(self::$structure,$name,NULL,'is_array');
        if(!is_array($element)) {
            throw new InvalidArgumentException("Undefined or invalid property [{$name}]!");
        }
        $access=get_array_value($element,'access','readonly','is_notempty_string');
        if($access=='private') {
            throw new InvalidArgumentException("Inaccessible property [{$name}]!");
        }
    ...
    public static function SetValue(string $name,$value): void {
        ...
        $validation=get_array_value($element,'validation','','is_string');
        if(strlen($validation) && !validate_param($value,NULL,$validation,TRUE)) {
