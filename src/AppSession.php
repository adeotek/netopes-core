...
class AppSession {
    ...
    public static function GetNewUID(?string $salt=NULL,string $algorithm='sha1',bool $notime=FALSE,bool $raw=FALSE): string {
        if($notime) {
            return hash($algorithm,$salt,$raw);
        }
    ...
    public static function ConvertToSessionCase($input,$keysCase=NULL) {
        ...
        if(is_array($input)) {
            ...
            foreach($input as $k=>$v) {
                $linput[$k]=self::ConvertToSessionCase($v,$keysCase);
            }
            return $linput;
        }//if(is_array($input))
        if(!is_string($input)) {
        ...
        if(AppConfig::GetValue('session_redis')===TRUE) {
            if(class_exists('\Redis',FALSE)) {
                try {
                    ...
                    if(is_string($session_name) && strlen($session_name)) {
    ...
    public static function SessionStart(string $path='',?bool $doNotKeepAlive=NULL,$ajax=FALSE) {
        ...
        if($doNotKeepAlive!==TRUE && $doNotKeepAlive!==1) {
    ...
    protected static function GetCustomParam($key,$path,$data) {
        ...
        if(is_array($path) && count($path)) {
            ...
            if(!strlen($lpath) || !array_key_exists($lpath,$data)) {
                return NULL;
            }
            return self::GetCustomParam($key,$path,$data[$lpath]);
        }//if(is_array($path) && count($path))
        if(is_string($path) && strlen($path)) {
            if(!array_key_exists($path,$data) || !is_array($data[$path])) {
                return NULL;
            }
    ...
    public static function GetGlobalParam($key,$phash=NULL,$path=NULL,$keysCase=NULL) {
        ...
        if($phash) {
            if(!array_key_exists($phash,self::$data)) {
                return NULL;
            }
            if(isset($lpath)) {
                return self::GetCustomParam($lkey,$lpath,self::$data[$phash]);
            }
            return (array_key_exists($lkey,self::$data[$phash]) ? self::$data[$phash][$lkey] : NULL);
        }//if($lphash)
        if($lpath) {
            return self::GetCustomParam($key,$lpath,self::$data);
        }
    ...
    public static function SetGlobalParam($key,$val,$phash=NULL,$path=NULL,$keysCase=NULL) {
        ...
        if(isset($lpath)) {
            if(is_array($lpath) && count($lpath)) {
                $part_arr=[$lkey=>$val];
                foreach(array_reverse($lpath) as $k) {
                    $part_arr=[$k=>$part_arr];
                }
                if($phash) {
            ...
            if(is_string($lpath) && strlen($lpath)) {
                if($phash) {
                    if(!isset(self::$data[$phash]) || !is_array(self::$data[$phash])) {
                        self::$data[$phash]=[];
                    }
                    if(!isset(self::$data[$lpath]) || !is_array(self::$data[$lpath])) {
                        self::$data[$lpath]=[];
                    }
    ...
    public static function MergeSession($current,$new,$overwrite=TRUE,$initial=NULL) {
        ...
        if(is_array($initial) && count($initial)) {
            foreach(array_diff_key($initial,$new) as $k=>$v) {
                unset($result[$k]);
            }
        }//if(is_array($initial) && count($initial))
        return $result;
    ...
    public static function GetParam(string $key,?string $phash=NULL,?string $namespace=NULL) {
        ...
        if(is_string($namespace)) {
            if(!array_key_exists($namespace,$data) || !is_array($data[$namespace])) {
                return NULL;
            }
            $data=$data[$namespace];
        }//if(is_string($namespace))
        if(is_string($phash)) {
            if(!array_key_exists($phash,$data) || !is_array($data[$phash])) {
                return NULL;
            }
            $data=$data[$phash];
        }//if(is_string($phash))
        $lkey=self::ConvertToSessionCase($key);
        if(!array_key_exists($lkey,$data)) {
            return NULL;
        }
        return $data[$lkey];
    ...
    public static function GetSessionAcceptedRequest(string $uid,?string $namespace=NULL) {
        if(strlen($namespace)) {
            return isset(static::$data[$namespace]['xURLRequests'][$uid]) ? static::$data[$namespace]['xURLRequests'][$uid] : NULL;
        }
    ...
    public static function SetSessionAcceptedRequest(?string $uid,?string $namespace=NULL): string {
        if(is_null($uid)) {
            $uid=static::GetNewUID(NULL,'md5');
        }
        if(strlen($namespace)) {
            if(!isset(static::$data[$namespace]) || !is_array(static::$data[$namespace])) {
                static::$data[$namespace]=[];
            }
            if(!isset(static::$data[$namespace]['xURLRequests']) || !is_array(static::$data[$namespace]['xURLRequests'])) {
                static::$data[$namespace]['xURLRequests']=[];
            }
            static::$data[$namespace]['xURLRequests'][$uid]=TRUE;
        } else {
            if(!isset(static::$data['xURLRequests']) || !is_array(static::$data['xURLRequests'])) {
                static::$data['xURLRequests']=[];
            }
    ...
    public static function CheckSessionAcceptedRequest(string $uid,bool $reset=FALSE,?string $namespace=NULL): bool {
        ...
        if($reset===TRUE) {
            static::UnsetSessionAcceptedRequest($uid,$namespace);
        }
        return ($result===TRUE);
