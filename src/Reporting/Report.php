...
class Report extends ExcelExport {
    ...
    public function __construct(&$params=[]) {
        ...
        foreach($params['layouts'] as $layout) {
            ...
            foreach($layout['columns'] as $column) {
                if(get_array_value($column,'hidden',FALSE,'bool')) {
                    $colnr++;
                    continue;
                }
                ...
                if(strlen($tr_val)) {
                    $total_row[$colnr]=['formula'=>$tr_val,'value'=>0];
                }
                $colnr++;
            ...
            foreach($data as $data_row) {
                ...
                foreach($layout['columns'] as $column) {
                    ...
                    if(strlen($format_string_call) && method_exists($this,$format_string_call)) {
                        $value=$this->$format_string_call($value);
                    }
                    if(strlen($value)) {
            ...
            if(is_array($total_row) && count($total_row)) {
                ...
                foreach($layout['columns'] as $colnr=>$column) {
                    ...
                    if(!array_key_exists($colnr,$total_row)) {
                        $bc_no++;
                        continue;
                    }
                    ...
                    if(strlen($tr_format_string_call) && method_exists($this,$tr_format_string_call)) {
                        $tr_value=$this->$tr_format_string_call($tr_value);
                    }
            ...
            if(strlen($ee_button)) {
                $this->result.="\t".'<div id="excelexport_errors"></div>'."\n";
            }
    ...
    protected function CreateCacheExcelFile(&$params) {
        if(!strlen($this->cached_file)) {
            return FALSE;
        }
        try {
            $cf_path=AppHelpers::GetCachePath().'reports';
            if(!file_exists($cf_path)) {
                mkdir($cf_path,0755,TRUE);
            }
            if(file_exists($cf_path.'/'.$this->cached_file)) {
                unlink($cf_path.'/'.$this->cached_file);
            }
            $params['output']=TRUE;
