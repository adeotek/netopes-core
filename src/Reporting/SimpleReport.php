...
class SimpleReport {
        ...
        foreach($layout as $layout_item) {
            ...
            foreach($data as $data_row) {
                ...
                foreach($layout_item['columns'] as $column) {
                    ...
                    if($is_sum[$colnr]) {
                        $sum_value[$colnr]+=floatval(str_replace($this->decimal_separator,'.',str_replace($this->group_separator,'',$value)));
                    }
            ...
            if($has_total) {
                ...
                if($blankcol) {
                    $total.="\t\t\t\t".'<td colspan="'.$blankcolspan.'">&nbsp;</td>'."\n";
                }
                $total.="\t\t\t".'</tr>'."\n";
                $total.="\t\t".'</tfoot>'."\n";
            }//if($has_total)
            $table_class=(array_key_exists('table_class',$layout_item) && strlen($layout_item['table_class'])>0) ? ' class="'.$layout_item['table_class'].'"' : ' class="'.$this->base_class.'"';
    ...
    protected function FormatValue(&$rowformat,$item,$formatcall=NULL,$class=NULL) {
        ...
        if(is_null($formatcall)) {
            $result='';
            foreach($rowformat['dbfield'] as $field) {
                $result.=' '.$item[$field];
            }
            return trim($result);
        }//if(is_null($formatcall)
    ...
    protected function DateFormat($value) {
        return Validator::ConvertDateTime($value,NApp::GetParam('timezone',FALSE),TRUE);
    }//END protected function DateFormat

    protected function DateTimeFormat($value) {
        return Validator::ConvertDateTime($value,NApp::GetParam('timezone',FALSE),FALSE);
